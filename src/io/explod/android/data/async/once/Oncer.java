...
/**
 * Oncer pool object. Stores results from one-time operations. You will use
 * Oncer.once and Oncer.resetKey for the bulk of your operations.
 * 
 * @param <KeyType>
 * @param <DataType>
 * @param <ErrorType>
...
public abstract class Oncer<KeyType, DataType, ErrorType> {
	...
	/**
	 * Run a callable only one time per key. Results from the first call are
	 * used. Can be called multiple times simultaneously and the runnable will
	 * be called only once.
	 * 
	 * @param key
	 * @param runnable
	...
	/**
	 * This method calls a callback with data and errors. Subclasses can perform
	 * the callback on a new thread, if they so desire.
	 * 
	 * @param data
	 * @param error
	 * @param callback
	 ...
	 * @param callback
	 */
	private synchronized void executeRunnable(final KeyType key, final OnceRunnable<DataType, ErrorType> runnable,
					final IResultCallback<DataType, ErrorType> callback) {
